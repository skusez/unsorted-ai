...
function TableHoverActionsContainer({
  ...
  const insertAction = (insertRow: boolean) => {
    editor.update(() => {
      if (tableDOMNodeRef.current) {
        ...
        if (maybeTableNode) {
          const colCount = (
            (maybeTableNode as TableCellNode).getChildAtIndex(0) as TableRowNode
          )?.getChildrenSize();
  ...
  return (
    <>
      {isShownRow && (
        <ToolTip label={"Insert row"} side="bottom">
          <Button
            size={"none"}
            className={
              "flex items-center  justify-center left-0 top-0 absolute  tableAddRows"
            }
            style={{ ...position }}
      ...
      {isShownColumn && (
        <ToolTip
          side="right"
          label={`${isColumLimit ? " reached Columns limit" : "insert Columns"}`}
        >
          <Button
            size={"none"}
            className={
              "flex items-center  justify-center left-0   top-0 absolute  tableColumns"
            }
            style={{ ...position }}
...
function getMouseInfo(event: MouseEvent): {
  ...
  if (target && target instanceof HTMLElement) {
    ...
    const isOutside = !(
      ...
      target.closest<HTMLElement>("button.tableColumns") ||

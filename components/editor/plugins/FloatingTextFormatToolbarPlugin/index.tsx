...
function TextFormatFloatingToolbar({
  ...
  const CommandList = [
    ...
    },
    {
      ...
      render: (
        <BlockFormatDropDown
          ActiveBlockName={blockTypeToBlockName[blockType] as string}
    ...
    },
    {
      label: "font-family",
      title: "font",
      render: <FontDropDown editor={editor} value={fontFamily} />,
    },
    {
      label: "Bold",
    ...
    >
      {CommandList.map((e) => {
        if (e.render) {
          if (e.title) {
            return (
              <ToolTip key={e.label} label={e.title}>
                {e.render}
              </ToolTip>
            );
          } else {
            return <React.Fragment key={e.label}>{e.render}</React.Fragment>;
          }
        } else {
          return (
            <ToolTip key={e.label} label={e.title} side="top">
              <ToolButton
                checked={e.active}
                onClick={e.command}
                style="h-[30px]"
